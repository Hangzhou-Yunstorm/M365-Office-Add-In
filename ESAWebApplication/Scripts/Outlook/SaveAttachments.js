...
(function () {
    ...
    Office.onReady(function () {
        $(document).ready(function () {
            ...
            if (isShowDefault()) {
                //初始话默认的文件路径
                var defaultFolder = localStorage.getItem("defaultAttachmentFolderName");
                var defaultFolderId = localStorage.getItem("defaultAttachmentFolderUrl");
...
function confirmSelect() {
    ...
    //__Attachments = Office.context.mailbox.item.attachments;
    __Attachments = Office.context.mailbox.item.attachments.filter(function (item, index, array) {
        return item.isInline == false;
    });

    if (__Attachments && __Attachments.length > 0) {
        ...
        if (typeSel == "2") {
            ...
            if (defaultFolderId) {
                ...
                window.location.href = "/Outlook?ReturnUrl=/Outlook/SaveAttachments";
            }
        } else if (typeSel == "3") {
            $(".file_type_select").hide();
            $(".SelectTypeAttachmentPath").hide();
            $(".SelectSavePath").show();
            $(".search-box").show();
            $(".all_folder").show();
        ...
        layer_alert(languageText.NoAttachments);
    }
}
...
function isShowDefault() {
    var isShow = false;
    // 默认文件路径开关
    var defaultFolderCKB = localStorage.getItem("defaultAttachmentFolderCKB");
...
function getEwsToken() {
    ...
    try {
        Office.context.mailbox.getCallbackTokenAsync({ isRest: true }, function (result) {
            __EwsToken = result.value;

            if (__Attachments && __Attachments.length > 0) {
                __currentAttachmentCount = 0;
                uploadFileToServer(1, __Attachments[__currentAttachmentCount].name);
            }
        });
    } catch (ex) {
...
function uploadFileToServer(ondup, fileName) {
    ...
    formdata.append('Docid', encodeURIComponent($("#saveFolderUrl").val()));
    formdata.append('AttachmentId', encodeURIComponent(__Attachments[__currentAttachmentCount].id));
    formdata.append('Ondup', ondup);
    formdata.append('EwsToken', encodeURIComponent(__EwsToken));
    formdata.append('EwsUrl', encodeURIComponent(Office.context.mailbox.ewsUrl));
    formdata.append('EwsId', encodeURIComponent(Office.context.mailbox.item.itemId));

    $.ajax({
        url: "/Outlook/SaveAttachmentToServer",
        type: "post",
        data: formdata,
        processData: false,
        contentType: false,
        ...
        success: function (data) {
            if (data.Success) {
                ...
                if (code == 0) {
                    __currentAttachmentCount++;
                    if (__currentAttachmentCount < __Attachments.length) {
                        uploadFileToServer(1, __Attachments[__currentAttachmentCount].name);
                    } else {
...
function replace() {
    hideDialog();
    index_layer = layer_load();
    uploadFileToServer(3, __Attachments[__currentAttachmentCount].name);
}
